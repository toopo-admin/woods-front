...
const schema: VbenFormSchema[] = [
  ...
  },
  {
    ...
    rules: z
      .string()
      .min(2, $t('ui.formRules.minLength', [$t('system.menu.menuCode'), 2]))
      .max(30, $t('ui.formRules.maxLength', [$t('system.menu.menuCode'), 30]))
      .refine(
        async (value: string) => {
          const res = await isMenuCodeExist(value, formData.value?.id);
          return res.valid;
        },
        (value) => ({
          message: $t('ui.formRules.alreadyExists', [
            $t('system.menu.menuCode'),
            value,
          ]),
        }),
      ),
  ...
  },
  {
    ...
    rules: z
      ...
      .refine(
        async (value: string) => {
          const res = await isMenuPathExist(value, formData.value?.id);
          return res.valid;
        },
        (value) => ({
          message: $t('ui.formRules.alreadyExists', [
